    ...
    <Card className="overflow-hidden border border-slate-100 dark:border-slate-800 bg-white/70 dark:bg-slate-900 backdrop-blur-md shadow-md shadow-slate-100/50 hover:shadow-lg hover:shadow-slate-100 transition-all duration-300">
        ...
        <div className="flex items-center space-x-3">
          <div
            className={`flex h-9 w-9 items-center justify-center rounded-xl border ${
              isPass
                ? "bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400 border-indigo-100/50 dark:border-indigo-900/50"
                : "bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 border-rose-100/50 dark:border-rose-900/50"
            }`}
          >
      ...
      <CardContent className="p-6 space-y-4">
        ...
        {!isPass && check.blocked_reason && (
          <div className="rounded-xl bg-rose-50 dark:bg-rose-950/30 border border-rose-100/50 dark:border-rose-900/50 p-4 flex items-start gap-2.5">
            <XOctagon className="h-5 w-5 text-rose-600 dark:text-rose-400 shrink-0 mt-0.5" />
            <div>
              <h4 className="text-sm font-semibold text-rose-900 dark:text-rose-200">Fairness Infraction Detected</h4>
              <p className="text-sm text-rose-700 dark:text-rose-300 mt-1 leading-relaxed">{check.blocked_reason}</p>
            </div>
          </div>
        )}
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div
            className={`rounded-xl border p-3.5 flex items-start gap-3 transition-colors ${
              check.forbidden_keywords_present
                ? "bg-rose-50/20 dark:bg-rose-950/30 border-rose-100 dark:border-rose-900/50"
                : "bg-emerald-50/10 dark:bg-emerald-950/30 border-emerald-100/50 dark:border-emerald-900/50"
            }`}
          >
            {check.forbidden_keywords_present ? (
            ...
            <div>
              <h4 className="text-sm font-bold text-slate-900 dark:text-slate-100">Protected Attributes Check</h4>
          ...
          >
            ...
            />
            <div>
              <h4 className="text-sm font-bold text-slate-900 dark:text-slate-100">Counterfactual Swapping</h4>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 leading-relaxed">
              ...
              <div className="mt-2.5">
                {check.counterfactual_agreed !== null ? (
                  ...
                  <Badge
                    variant="outline"
                    className="text-[10px] font-bold uppercase text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-800"
                  >
                    Not run yet
                  </Badge>
                )}
